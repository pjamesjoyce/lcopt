...
(function(root, factory) {
	...
	$.extend(Selectize.prototype, {
		...
		//jj
		getElementWithUniqueValue: function(value, $els) {
			value = hash_key(value);
	
			if (value) {
				for (var i = 0, n = $els.length; i < n; i++) {
					if ($els[i].getAttribute('data-uniquevalue') === value) {
						return $($els[i]);
					}
				}
			}
		...
		getUniqueItem: function(value) {
			return this.getElementWithUniqueValue(value, this.$control.children());
		},
		...
		addItem: function(value) {
			debounce_events(this, ['change'], function() {
				...
				//jj
				var random_suffix = Math.floor(Math.random() * 99999999999);
				var unique_value = value + random_suffix;
				
				$item = $(self.render('item', self.options[value], unique_value)); // jj change to render function
				self.items.splice(self.caretPos, 0, value);
				//jj
				self.unique_items.splice(self.caretPos, 0, unique_value);
		...
		removeItem: function(value) {
			...
			$item = (typeof value === 'object') ? value : self.getUniqueItem(value);
			//value = hash_key($item.attr('data-value'));
			//i = self.items.indexOf(value);
			//jj

			value = hash_key($item.attr('data-uniquevalue'));
			//console.log(value)
			i = self.unique_items.indexOf(value);
			//console.log('delete item at position '+ i)
		...
		deleteSelection: function(e) {
			...
			////console.log('this is the deleteSelection function')
			////console.log('e = ')
			////console.log(e)
			...
			//console.log(self.caretPos)
			//console.log(self.items)
			//console.log(self.unique_items)
			...
			if (self.$activeItems.length) {
				...
				for (i = 0, n = self.$activeItems.length; i < n; i++) {
					...
					//jj
					unique_values.push($(self.$activeItems[i]).attr('data-uniquevalue'));
				}
				if (e) {
					e.preventDefault();
					e.stopPropagation();
				...
				if (direction < 0 && selection.start === 0 && selection.length === 0) {
					...
					//jj
					unique_values.push(self.unique_items[self.caretPos - 1]);
				} else if (direction > 0 && selection.start === self.$control_input.val().length) {
					values.push(self.items[self.caretPos]);
					//jj
					unique_values.push(self.unique_items[self.caretPos]);
				}
			}
			...
			//}
			while(unique_values.length){
				self.removeItem(unique_values.pop());
			}
		 ...
		 * @returns {string}
		 */
		render: function(templateName, data, unique_value) {
			...
			//jj
			cache = false; // force cache to false
			...
			if (templateName === 'option' || templateName === 'item') {
				...
				html = html.replace(regex_tag, '<$1 data-value="' + escape_replace(escape_html(value || '')) + '" data-uniquevalue="' + escape_replace(escape_html(unique_value || '')) + '"');
			}
